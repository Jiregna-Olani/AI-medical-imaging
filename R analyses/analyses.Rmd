...
```{r echo=FALSE}
# Clean and standardize the ML type Used values
data <- data %>%
  mutate(across(
...
# Combine the two columns into one long format
...
# Plot the frequency distribution (cleaned)
ggplot(ml_type_counts, aes(x = reorder(ML_Type, n), y = n)) +
  geom_bar(stat = "identity", fill = "darkred") +
  coord_flip() +
  geom_text(aes(label = Percentage_Label), hjust = -0.1, size = 3.5) +
  labs(
    title = "Frequency Distribution of AI Techniques",
    x = "Type of AI Used",
    y = "Frequency counts with percentage"
  ) +
...
# Extract publication year from study_id_2
data$publication_year <- str_extract(data$study_id_2, "\\b(19|20)\\d{2}\\b") %>% as.numeric()
